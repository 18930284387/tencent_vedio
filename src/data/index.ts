import { Video, Banner, tabs, categories } from '../types';

export { tabs, categories };
...
export const filterVideos = (videoList: Video[], category: string): Video[] => {
  if (category === '全部') return videoList;
  if (category === 'VIP') return videoList.filter(v => v.isVip);
  if (category === '免费') return videoList.filter(v => !v.isVip);
  if (category === '热门') return videoList.filter(v => v.rating && v.rating >= 9);
  if (category === '最新') return videoList.filter(v => v.id > 5);
  if (category === '好评') return videoList.filter(v => v.rating && v.rating >= 8.5);
  return videoList;
};

export const getCategoryVideos = (videoList: Video[], category: string): Video[] => {
  return videoList.filter(v => v.category === category).slice(0, 6);
};

export const searchVideos = (videoList: Video[], query: string): Video[] => {
  if (!query) return [];
  return videoList.filter(v => v.title.toLowerCase().includes(query.toLowerCase()));
};
